...
class FinnHubExchange(GenericRestExchange):
    ...
    async def get_quote(self, symbol: str) -> Quote:
        ...
        data = await self._request("GET", "/quote", params={"symbol": symbol.upper(), "token": self.api_key})
    ...
    async def stream_quotes(self, symbols: List[str]) -> AsyncIterable[Quote]:
        ...
        raise NotImplementedError("FinnHub quotes streaming requires WebSocket implementation.")

    async def stream_trades(self, symbols: List[str]) -> AsyncIterable[Trade]:
        """FinnHub requires a WebSocket implementation (not REST)."""
        raise NotImplementedError("FinnHub trades streaming requires WebSocket implementation.")
